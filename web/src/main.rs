            ...
            .await
        {
            Ok(response) if response.status().is_success() => {
                match response.json::<serde_json::Value>().await {
                    Ok(llama_response) => {
                        println!("成功从llama.cpp获取响应");

                        // 提取响应文本
                        let response_text = if let Some(content) = llama_response["content"].as_str() {
                            content.to_string()
...
async fn check_connection() -> Result<HttpResponse> {
    ...
    for endpoint in llama_endpoints.iter() {
        match client.get(*endpoint).timeout(Duration::from_secs(3)).send().await {
            Ok(response) => {
                results.push(format!("llama.cpp {}: ✅ 成功 (状态: {})", endpoint, response.status()));
            }
            Err(e) => {
                results.push(format!("llama.cpp {}: ❌ 失败 ({})", endpoint, e));
            }
        }
    }
